...
for x in tqdm(SKTCRI):
    ...
    for z in range(len(DDW[str(x)])):
        ...
        if DDW[str(x)]["K"].iloc[z] > DDW[str(x)]["D"].iloc[z] and DDW[str(x)]["K"].shift(1).iloc[z] < DDW[str(x)]["D"].shift(1).iloc[z]         and DDW[str(x)]['成交金額(千)'].iloc[z] < 500000          and DDW[str(x)]['D'].iloc[z] < 20 and DDW[str(x)]['還原收盤價'].iloc[z] < (DDW[str(x)]['MAX52S']*0.7).iloc[z] :
